...
def _calc_action_reward(self, prev_action):
    # action reward
    action = self.robot.get_pd_target()[0]
    penalty = 5 * sum(np.abs(prev_action - action)) / len(action) # NOTE prev_action - action = 0. THIS IS A BUG FROM THE ORIGINAL CODE.
    # THIS IS USELESS AS IT IS. I WILL CHANGE IT TO THE FOLLOWING:
    return np.exp(-penalty)

def _calc_torque_reward(self, prev_torque):
    # actuator torque reward
    torque = np.asarray(self.robot.get_act_joint_torques())
    penalty = 0.25 * (sum(np.abs(prev_torque - torque)) / len(torque))
    return np.exp(-penalty)

def _calc_height_reward(self):
    # height reward
    # if self.robot.check_rfoot_floor_collision() or self.robot.check_lfoot_floor_collision():
    #     contact_point = min([c.pos[2] for _,c in (self.robot.get_rfoot_floor_contacts() +
...
def _calc_heading_reward(self):
    # heading reward
    cur_heading = self.robot.get_qvel()[:3]
...
def _calc_foot_frc_clock_reward(self, left_frc_fn, right_frc_fn):
    ...
    desired_max_foot_frc = self.robot.get_robot_mass()*9.8*0.5
    #desired_max_foot_frc = self.robot.get_robot_mass()*10*1.2
...
def _calc_foot_pos_clock_reward(self):
    ...
    l_foot_pos = self.robot.get_object_xpos_by_name('lf_force', 'OBJ_SITE')[2]
    r_foot_pos = self.robot.get_object_xpos_by_name('rf_force', 'OBJ_SITE')[2]
...
def _calc_joint_vel_reward(self, enabled, cutoff=0.5):
    ...
    motor_speeds = self.robot.get_motor_velocities()
    motor_limits = self.robot.get_motor_speed_limits()
...
def _calc_joint_acc_reward(self):
    ...
    joint_acc_cost = np.sum(np.square(self.robot.get_qacc()[-self._num_joints:]))
    return self.wp.joint_acc_weight*joint_acc_cost

def _calc_ang_vel_reward(self):
    # angular vel reward
    ang_vel = self.robot.get_qvel()[3:6]
    ang_vel_cost = np.square(np.linalg.norm(ang_vel))
    return self.wp.ang_vel_weight*ang_vel_cost

def _calc_impact_reward(self):
    # contact reward
    ncon = len(self.robot.get_rfoot_floor_contacts()) + \
           len(self.robot.get_lfoot_floor_contactts())
    if ncon==0:
        return 0
    quad_impact_cost = np.sum(np.square(self.robot.get_body_ext_force()))/ncon
...
def _calc_foot_contact_reward(self):
    right_contacts = self.robot.get_rfoot_floor_collisions()
    left_contacts = self.robot.get_lfoot_floor_collisions()

    radius_thresh = 0.3
    f_base = self.robot.get_qpos()[0:2]
...
def _calc_gait_reward(self):
    ...
    rfoot_grf = self.robot.get_rfoot_grf()
    lfoot_grf = self.robot.get_lfoot_grf()

    # get foot speed
    rfoot_speed = self.robot.get_rfoot_body_speed()
    lfoot_speed = self.robot.get_lfoot_body_speed()

    # get foot position
    rfoot_pos = self.robot.get_rfoot_body_pos()
    lfoot_pos = self.robot.get_lfoot_body_pos()
    swing_height = 0.3
    stance_height = 0.1
...
def _calc_reference(self):
    ...
    # get current pose
    current_pose = np.array(self.robot.get_act_joint_positions())
